...
### 3. 2026年8月の横浜市「完成イメージ」を立体構造と合わせて読む

![2026年完成イメージの模式図](docs/generated/06-official-2026-schematic.png)

横浜市の2026年8月工事説明会資料では、三ツ境駅前からの既存道路と新設道路を接続するために1号交差点の形状を変更し、**交通島、新たな横断歩道、信号**を整備する完成イメージが示されています。

この図では、その地上部分だけを説明用に単純化し、右側に現況OSMで確認した上下関係を併記しています。県道45号のアンダーパスと相鉄線を、地上の1号交差点と同じ平面には描いていません。

**車線数・横断歩道の厳密な位置・信号灯器の最終配置を復元した図ではありません。**

### 4. 2018年に公表された信号パターン検討案
...
GitHub Actions が公開OSMデータからSVGを生成し、PNGへ決定論的にレンダリングした後、SVGの構造検査を実行します。2026年完成イメージの模式図も同じCIで生成・検査します。SVGを編集可能なmasterとし、PNGはレビュー用成果物として扱います。
